...
//   underline: { height: vs(5), width: s(60), backgroundColor: COLORS.primary ,marginTop:ms(2)},
// });

//   // ─── News Card (same as CommonSectionScreen) ────────────────────────────────────────
// function DinamDinamNewsCard({ item, onPress, sectionTitle = '' }) {
//   const { sf } = useFontSize();
...
// // HTML helpers from CommonSectionScreen
...
// // InlineVideoPlayer component from CommonSectionScreen
// function InlineVideoPlayer({ url, style }) {
//   return (
//     <View style={style}>
//       <Text style={{ textAlign: 'center', color: '#666', marginTop: vs(10) }}>Video: {url}</Text>
//     </View>
//   );
// }
...
//       );
//     }
//     return <DinamDinamNewsCard item={row.item} onPress={() => goToArticle(row.item)} sectionTitle={row.sectionTitle || ''} />;
//   };
...
const DinamDinamScreen = () => {
  return (
    <View>
      <Text>DinamDinamScreen</Text>
    </View>
  )
}

export default DinamDinamScreen