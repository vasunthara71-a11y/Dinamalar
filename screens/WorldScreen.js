...
export default function WorldScreen() {
  ...
  // ─── Fetch World News ───────────────────────────────────────
  const fetchWorldNews = useCallback(async (pageNum = 1, isRefresh = false) => {
    try {
      if (pageNum === 1) {
      ...
      // Use correct API endpoint for World news (category ID 34)
      const response = await mainApi.get('/newsdata?cat=34');
      const newsData = response.data?.detail || [];

      console.log('World news response:', response.data);
  ...
  const onRefresh = useCallback(() => {
    setHasMore(true);
    fetchWorldNews(1, true);
  }, [fetchWorldNews]);

  const onEndReached = useCallback(() => {
    if (!loadingMore && hasMore && !loading) {
      fetchWorldNews(page + 1);
    }
  }, [loadingMore, hasMore, loading, page, fetchWorldNews]);
        ...
        <View style={styles.newsContent}>
          ...
          <View style={styles.categoryBadge}>
            <Text style={styles.categoryText}>
              {item.categrorytitle || item.catengtitle || 'உலகம்'}
            </Text>
          </View>
  ...
  const renderEmptyState = () => {
    ...
    return (
      <View style={styles.emptyState}>
        ...
        <Text style={styles.emptyStateTitle}>உலகம் செய்திகள்</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>உலகம்</Text>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => `world-${item.id || item.newsid || index}`}
