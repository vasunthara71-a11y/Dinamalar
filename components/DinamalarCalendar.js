...
function SectionHeader({ title, onSeeMore }) {
  ...
  return (
    <TouchableOpacity
      ...
      disabled={!onSeeMore}
    >
      <Text style={[localStyles.sectionTitle, { fontSize: sf(18) }]}>{title}</Text>
      <View style={localStyles.sectionUnderline} />
    </TouchableOpacity>
  );
}
...
export default function DinamalarCalendarScreen() {
  ...
  return (
    <SafeAreaView style={st.safe}>
      ...
      <TouchableOpacity
        style={st.calendarContainer}
        onPress={() => {
          console.log('📅 CALENDAR CLICKED - Opening Dinamalar Calendar Play Store');
          const playStoreLink = 'https://play.google.com/store/apps/details?id=com.daily.dinamalar';
          Linking.openURL(playStoreLink).catch(() => console.log('Failed to open calendar Play Store link'));
        }}
        activeOpacity={0.8}
      >
        <ScrollView
        ...
        >
          ...
          <SectionHeader 
            title="காலண்டர்"
            onSeeMore={() => {
              console.log('📅 CALENDAR TITLE CLICKED - Opening Dinamalar Calendar Play Store');
              const playStoreLink = 'https://play.google.com/store/apps/details?id=com.daily.dinamalar';
              Linking.openURL(playStoreLink).catch(() => console.log('Failed to open calendar Play Store link'));
            }}
          />
