...
const DrawerMenu = ({ isVisible, onClose, onMenuPress, navigation }) => {
  ...
  const handleMenuItemPress = (item, parentItem = null) => {
    ...
    if (isCalendarParent) { navigation?.navigate('CommonSectionScreen', { screenTitle: 'காலண்டர்', apiEndpoint: '/calendar', allTabLink: '/calendar', initialTabId: id || '', initialTabLink: link || '', initialTabTitle: title || '' }); onClose(); return; }
                        ...
                        {hasSub && isExpanded && (
                          <View style={ds.subList}>
                            {validSubItems.map((sub, si) => {
                              ...
                              const isAllRow = subTitle === 'All';
                              return (
                                <TouchableOpacity
                                  key={`sub-${index}-${si}`}
                                  style={[ds.subRow, isAllRow && ds.subRowAll]}
                                  onPress={() => isAllRow ? handleMenuItemPress(item) : handleMenuItemPress(sub, item)}
                                  activeOpacity={0.7}
                                >
                                  <Text
                                    style={{
